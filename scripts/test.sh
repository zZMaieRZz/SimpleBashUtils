...
echo -e "-------------------------------------------------------------------------------\n"
echo -e "Style test\n"
STYLE_TEST_SCRIPT=scripts/style_tests.sh
...
for TEST_NUMBER in $(jq ".tests[].number" ${TEST_JSON}); do
  ...
  then
    echo -e "Part:" ${NAME}"\n"
    LAST_PART=$PART
    TEST_SCRIPT=scripts/integration_test.sh
    OUTPUT=$(bash ${TEST_SCRIPT} ${NAME})
    TEST_RESULT=${OUTPUT: -1}
    printf 'Integration test output:\n %s\n' "$OUTPUT"
